...
|        `secretStorage.mode`         |                      `'auto' \| 'keychain' \| 'filesystem'`                       | Mode of secret storage. See [Secret Storage Modes](#secret-storage-modes) for more details.                                                                                                      |        `'auto'`         |
| `secretStorage.encryptionKeyEnvVar` |                                  `string`                                   | Name of the environment variable that contains the encryption key for file-based secret storage. Only used if `secretStorage.mode` is set to `'filesystem'` or `'auto'` and keychain is not available. | `'CLI_CORE_VAULT_KEY'`  |
